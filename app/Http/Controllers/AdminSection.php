...
class AdminSection extends Controller
{
    ...
    public function destroy($id)
    {
        ...
        foreach ($services as $service) {
            $workExists = Work::where('service_id', $service->id)->exists();
            if ($workExists) {
                $hasWork = true;
                break;
            }
        ...
        if ($hasWork) {
            session()->flash('fail_delete_section', 'Deletion is not allowed. This section contains services, these services have works assigned to suppliers.');
