...
<body id="page-top">
                ...
                <div style="margin-top: -22px;" id="chat-box" class="chat-box">
                    <ul class="chat-list">
                        ...
                        <li class="chat-bubble {{ $message->receiver_id != 0 ? 'bubble-sender' : 'bubble-receiver' }} {{ $message->receiver_id != 0 ? 'align-end text-right' : 'align-start text-left' }}">
                            ...
                            {{ $message->receiver_id != 0 ?   'You' : $client->name}}
                            </strong><br> 
                            @if($message->message)
                            {{ $message->message }}
                            @endif
    ...
    <script>
        ...
        const channel = pusher.subscribe('ClientToAdmin');
        channel.bind('ClientAdmin-message', function(data) {
            console.log('Received message:', data);

            // التحقق من أن الرسالة موجهة لنا
            if (clientId == data.sender_id && data.role == 'client') {
