...
<body>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              ...
              <form id="delete-form-{{ $work->id }}" action="{{ route('Supplier.Delete.Work', $work->id) }}" method="POST" style="display: none;">
                @csrf
                @method('DELETE')
              </form>
              <script>
  ...
  <script>
    Notiflix.Notify.failure("{{ session('error_Create') }}");
  </script>
  @endif
  @if(session('error_delete_work_order'))
  <script>
    Notiflix.Notify.failure("{{ session('error_delete_work_order') }}");
  </script>
  @endif
</body>
