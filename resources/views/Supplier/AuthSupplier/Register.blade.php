...
<html lang="en">
  ...
  <header>Supplier Registration</header>
  <form action="{{ route('Store.account.Supplier') }}" method="POST" class="form">
    @csrf
    <div class="input-box">
    ...
    <div class="gender-box">
      ...
      <div class="gender">
        <input type="radio" id="status-student" name="status" value="student" {{ old('status') == 'student' ? 'checked' : '' }} />
        <label for="status-student">Student</label>
      </div>
      <div class="gender">
        <input type="radio" id="status-employed" name="status" value="employed" {{ old('status') == 'employed' ? 'checked' : '' }} />
        <label for="status-employed">Employed</label>
      </div>
      @error('status')
        <span class="text-danger">{{ $message }}</span>
      @enderror
